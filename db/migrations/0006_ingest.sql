...
-- Metadata versioning. `field`+`old`/`new` is the whole table's purpose: an alert must be able to say "the end
...
CREATE INDEX IF NOT EXISTS tape_fills_market_time_idx ON tape_fills (market_id, ts_ms DESC);
