...
class AutomationEngine:
    ...
    def facts_for(self, *, user_id: str, market_id: str, token_id: str, at: int) -> Facts:
        """Everything a trigger may look at for one (rule, market) pair, read from our own tables. The engine
        never queries the venue for facts: the ingest plane is the only thing that talks to Polymarket, and a
        rule that fetched its own price would be a rule with its own freshness bug."""
        if not market_id:
            return Facts(now_ms=at, accepting_orders=True, quote_age_ms=0)
        q = self.store.market_quote(market_id, at=at)
        row = self.store.conn.execute("SELECT accepting_orders, first_seen_ms, end_ts FROM markets WHERE id=?",
                                      (market_id,)).fetchone()
        pos = self.store.conn.execute("SELECT COALESCE(SUM(shares_open_micro),0), "
                                      "COALESCE(SUM(basis_micro),0) FROM position_lots WHERE user_id=? AND "
                                      "market_id=? AND shares_open_micro>0", (user_id, market_id)).fetchone()
        shares = int(pos[0] or 0) if pos else 0
        basis = int(pos[1] or 0) if pos else 0
        avg = (basis * 10**SCALE // shares) if shares > 0 and basis else None
        imbalance = 0
        depth = self.store.conn.execute("SELECT side, COALESCE(SUM(size_shares_micro),0) FROM book_levels "
                                        "WHERE market_id=? GROUP BY side", (market_id,)).fetchall()
        # `book_levels.side` is 'bid'/'ask' (lowercase, the venue's own words), not 'BUY'/'SELL'. Getting this
        # wrong does not throw: it returns 0 depth on both sides, an imbalance of 0, and a book_imbalance
        # rule that never fires. `store.market_quote` is the reference for the spelling.
        ...
        return Facts(last_price_micro=q.get("last_price_micro"), best_bid_micro=q["best_bid_micro"],
                     ...
                     accepting_orders=bool(row[0]) if row else False,
                     imbalance_bps=imbalance, position_shares_micro=shares, avg_entry_micro=avg)
