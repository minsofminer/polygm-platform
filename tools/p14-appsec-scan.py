...
PATH_ALLOW = (
    ...
    ("tools/secret-scan-allowlist.json", "the exemption file itself: its whole content is the *text* of entries "
                                         "that must not be secrets, quoted so both scanners can match them. A scan "
                                         "that fails on the file listing its own exemptions fails on every change "
                                         "to it, which teaches the reader to ignore this check. The path entries in "
                                         "this file are still enforced by `tools/ci-log-scan.py`, whose own "
                                         "allowlist is separate code"),
    ("tools/sast-triage.json", "generated triage: it quotes rule messages and file paths, and is regenerated by "
                               "`tools/build-sast-triage.py` (whose output `--check` compares)"),
)
