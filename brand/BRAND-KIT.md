...
Rename note: `PolyGM` was the working name. The rename to **Openout** was executed per
`../prompts/P02-branding.md` D2, and it is machine-enforced, not hand-applied:
`node tools/rename-wordmark.mjs <old> <new>` rebuilds the lockup from `svg/mark.svg` and refuses
to write if any geometry, metric, or fallback differs from the fixed mark. Re-run
`--check` in any doubt (exit 1 = the old name is still live, or the mark has drifted).
...
```json
{
  "version": 2,
  "brand_lock": {
    "geometry_sha256": "c33b4d5fd82b7acd",
    "comment": "canonical SHA-256(16) of the mark's shape elements in mark.svg's 100-unit space. rename-wordmark.mjs refuses to write unless the lockup matches this, and re-checks it after writing. Regenerate with: node tools/rename-wordmark.mjs --hash"
  },
  "brand_context": {
    "name": "Openout",
  ...
  "authoritative_assets": {
    "logo": {
      ...
      "lockup_generation": "brand/svg/lockup-horizontal.svg is GENERATED by tools/rename-wordmark.mjs; never hand-edit it",
      "variants": {
        "color": "svg/mark.svg",
...
| `svg/lockup-horizontal.svg` | mark + wordmark, type set in code | fixed (text swaps on rename) — **generated**, see `tools/rename-wordmark.mjs` |
...
| `app-icon-512.png` | app icon, rounded-square | fixed — **actual size 1254×1254**, name overstates nothing but the spec wants 512 |
| `avatar-512.png` | Telegram/Discord avatar, circular-crop safe | fixed — **actual size 1254×1254** |
| `og-1200x630.png` | social sharing card | fixed — **actual size 1731×909**; aspect 1.904:1 (spec 1.91:1), needs a true 1200×630 derivative |
| `wordmark-metrics.json` | **new (P03)** measured hmtx advances that size the lockup viewBox | proposed — regenerate with `python3 tools/wordmark-width.py <Name> --json` |
